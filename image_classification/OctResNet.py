...
from octconv import OctaveConv, Conv_BN, Conv_BN_ACT

class Oct_BottleNeck(nn.Module):
    expansion = 4

    def __init__(self, inplanes, planes, stride = 1, downsample = None, groups = 1, base_width = 64,
                alpha_in = 0.5, alpha_out = 0.5, norm_layer = None, output = False):
        super(Oct_BottleNeck, self).__init__()
...
class Oct_ResNet(nn.Module):
    ...
    def __init__(self, block, layers, num_classes = 1000, zero_init_residual = False,
        ...
        if zero_init_residual:
            for m in self.modules():
                if isinstance(m,Oct_BottleNeck):
...
def octresnet50(pretrained = False, **kwargs):
    ...
    model = Oct_ResNet(Oct_BottleNeck, [3, 4, 6, 3], **kwargs)
    return model